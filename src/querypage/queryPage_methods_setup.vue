...
export default defineComponent({
  ...
  setup() {
    ...
    let conditionColumns = ref([]);
    let operBtn = ref([]);
    let tableColumns = reactive([]);
    let operColumn = ref([]);
    let templateColumnData = ref({});
    let tableData = ref([]);
    let selectRows = ref([]);
    let pageNo = ref(1);
    let pageSize = ref(10);
    let totalRecord = ref(0);
    let pageController = ref("");
    ...
    const getTableData = (queryParams: any) => {
      loading.value = true;
      console.log("querypage queryParams", queryParams);
    ...
    const btnfunc = (btn) => {
      if (btn.funName) {
        ...
        if (btn.needCheckbox == 1) {
          queryPageCtrl[pageController.value][btn.funName].call(
            this,
            selectRows
          );
        }
    ...
    const linkJump = (funName, row) => {
      queryPageCtrl[pageController][funName](row);
    };
    //表格方法
    ...
    const operColumnFunc = (index, row, typeIndex) => {
      queryPageCtrl[pageController.value][
